...
export default function Button({ href, text, className = '', bgColors, textColor }: ButtonProps) {
  ...
  return (
    <div className="relative inline-block overflow-hidden group">
      <Link
        href={href}
        className={`inline-flex items-center justify-center ${defaultBgColor} ${defaultTextColor} px-4 py-2 rounded-full transition-all duration-500 ease-in-out ${hoverBgColor} ${hoverTextColor} ${className}`}
      >
